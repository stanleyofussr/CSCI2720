...
class LogIn extends React.Component {
	...
	render() {
		return (
			<span className="logIcon" onMouseEnter={(e) => this.enter(e)} onMouseLeave={(e) => this.leave(e)} onClick={this.props.login}>
...
class LogOut extends React.Component {
	...
	render() {
		return (
			<span className="logIcon" onMouseEnter={(e) => this.enter(e)} onMouseLeave={(e) => this.leave(e)} onClick={this.props.logout}>
				<svg focusable="false" viewBox="0 0 512 512" width="28px">
					<title>{"Log Out"}</title>
		...
		if(this.state.admin == null)
			return (
				<span>
					...
					{this.state.username == null ? <LogIn login={this.login}/> : <LogOut logout={this.logout}/>}
				</span>
			);
		return (
			<span>
